...
class HistoryViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val monthStats: StateFlow<MonthStats> = combine(
        currentMonth, settings, repo.records
    ) { month, s, _ ->
        computeMonthStats(month, s.dailyGoalMl)
    ...
    fun nextMonth() {
        ...
        if (_currentMonth.value.isBefore(now)) {
            _currentMonth.value = _currentMonth.value.plusMonths(1)
        }
    }

    private fun computeMonthStats(month: YearMonth, goalMl: Int): MonthStats {
